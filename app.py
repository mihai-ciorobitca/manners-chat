...
def home():
    if 'username' in session:
        ...
        if session["username"] != 'admin':
            ...
            groups = supabase_client.from_('groups').select('groupname,sharelink').execute().data
...
def get_messages(data):
    ...
    groupmessage = supabase_client.from_("groups").select("groupmessage").eq("groupname", groupname).execute().data
...
def update_message():
    ...
    supabase_client.table("groups").update({"groupmessage": groupmessage}).eq("groupname", groupname).execute()
    session["groupmessage"] = groupmessage
    return redirect("/")
...
def reset():
    if request.method == "POST":
        ...
        users = supabase_client.table('users').select("*").eq('otp', otp).execute().data
        if len(users) > 0:
            user = users[0]
            ...
            if totp.verify(secret):
                ...
                supabase_client.table("users").update({"password": generate_password_hash(new_password)}).eq("username", user["username"]).execute()
                return redirect("/login")
